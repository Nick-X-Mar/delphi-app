...
export async function GET(request, { params }) {
  try {
    ...
    const result = await pool.query(
      `SELECT DISTINCT b.*, p.first_name, p.last_name, p.email, p.salutation, p.company, p.room_type, p.companion_full_name, h.name as hotel_name, h.hotel_id, h.phone_number, h.website_link, rt.name as room_type_name
      FROM bookings b
