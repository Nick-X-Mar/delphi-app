    ...
    try {
        ...
        try {
            for (const update of updates) {
                const { date, available_rooms, price_per_night, single_price_per_night } = update;
                const singlePrice = single_price_per_night != null ? single_price_per_night : null;
                    ...
                    result = await client.query(
                        `UPDATE room_availability 
                         SET available_rooms = $1, 
                             price_per_night = $2,
                             single_price_per_night = $3
                         WHERE room_type_id = $4 
                         AND date = $5::date
                         RETURNING room_type_id, date::text, available_rooms, price_per_night::numeric, single_price_per_night::numeric`,
                        [available_rooms, price_per_night, singlePrice, roomTypeId, date]
                    );
                } else {
                    result = await client.query(
                        `INSERT INTO room_availability 
                         (room_type_id, date, available_rooms, price_per_night, single_price_per_night)
                         VALUES ($1, $2::date, $3, $4, $5)
                         RETURNING room_type_id, date::text, available_rooms, price_per_night::numeric, single_price_per_night::numeric`,
                        [roomTypeId, date, available_rooms, price_per_night, singlePrice]
                    );
                }
