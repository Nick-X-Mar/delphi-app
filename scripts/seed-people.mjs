...
async function seedPeople() {
  ...
  try {
    ...
    const dbInfo = await client.query('SELECT current_database() as db_name, current_schema() as schema_name');
    console.log('Connected to database:', dbInfo.rows[0].db_name);
    console.log('Current schema:', dbInfo.rows[0].schema_name);
    
    const tablesCheck = await client.query(`
      SELECT table_name 
      FROM information_schema.tables 
      WHERE table_schema = 'public' AND table_name IN ('people', 'people_details', 'events', 'event_people')
      ORDER BY table_name
    `);
    console.log('Available tables:', tablesCheck.rows.map(r => r.table_name));
    
    // Check columns in people_details table
    const columnsCheck = await client.query(`
      SELECT column_name, data_type 
      FROM information_schema.columns 
      WHERE table_schema = 'public' AND table_name = 'people_details'
      ORDER BY ordinal_position
    `);
    console.log('people_details columns:', columnsCheck.rows.map(r => r.column_name));
    
    await client.query('BEGIN');
    
    // Get the maximum person_id to start from
    const maxIdResult = await client.query('SELECT COALESCE(MAX(person_id), 0) as max_id FROM public.people');
    ...
    // Get the latest event to optionally link people
    const eventsResult = await client.query('SELECT event_id FROM public.events ORDER BY event_id DESC LIMIT 1');
    ...
    for (let i = 0; i < samplePeople.length; i++) {
      ...
      if (peopleResult.rows.length > 0) {
        ...
        const insertDetailsQuery = `
          INSERT INTO public.people_details (
            person_id,
            room_size,
            notes,
            updated_at
        ...
        const detailsValues = [
          personId,
          roomSize,
          `Sample person for testing - ${person.first_name} ${person.last_name} (${person.job_title} at ${person.company})`,
          currentTimestamp
        ];
