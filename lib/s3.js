...
const s3Client = new S3Client({
  ...
  credentials: isDev 
    ? fromIni({
        ...
        profile: 'delphi-role'  // Using the terraform role profile that we know works
      })
    : undefined,  // In production, let it use the Amplify role automatically
  maxAttempts: 3,  // Add retry mechanism
  retryMode: 'adaptive'  // Automatically adjust retry timing
});

// Test the connection immediately
if (isDev) {
  console.log('[S3] Testing connection with TerraformExecutionRole...');
