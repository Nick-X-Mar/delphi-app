...
export const VALID_FUNDING_TYPES = ['self_funded', 'forum_covered'];
...
export function validatePerson(person, { accommodationStartDate, accommodationEndDate } = {}) {
  ...
  if (!person.guest_type || String(person.guest_type).trim() === '') {
    errors.push('guest_type is required');
  }
  ...
  if (person.accommodation_funding_type && !VALID_FUNDING_TYPES.includes(String(person.accommodation_funding_type).toLowerCase())) {
    errors.push('accommodation_funding_type must be "self_funded" or "forum_covered"');
  }
